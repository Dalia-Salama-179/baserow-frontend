...
import GroupService from '@baserow/modules/core/services/users'
import {
  setGroupCookie,
  unsetGroupCookie,
...
function populateUsers(users) {
  users._ = { loading: false, selected: false }
  return users
}
...
export const actions = {
  ...
  async createUser({ commit, dispatch }, values) {
    const { data } = await GroupService(this.$client).createUser(values)
    dispatch('forceCreateUser', data)
    return data
  },
  /**
  ...
  forceCreateUser({ commit }, values) {
    commit('ADD_ITEM_USER', values)
  },
  /**
