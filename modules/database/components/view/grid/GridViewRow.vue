              ...
              'grid-view__row-info--matches-search':
                ...
                row._.fieldSearchMatches.includes('row_id'),
                    ...
                    >
                        ...
                        ></component>
                        <!-- v-show="row._.hover" -->
                        <button v-if="table.name == 'organizations'" @click="getCrunchBase(row)" title="Get crunch base"
    ...
    methods: {
      async getCrunchBase(row) {
        ...
        if (selectedRows && selectedRows.length) {
          selectedRows.map(r => {
            this.$client.post(
              `t2/crunch_base_organization/${this.table.id}/${r.id}/`,
              {
          ...
          selectedRows.map(r => {
            this.$client.post(
              `t2/crunch_base_founder/${this.table.id}/${r.id}/`,
              {
                ...
                'organization_of_interest_cb_link': 'field_367'
              }
            ).then(() => {
              let refresh = JSON.parse(localStorage.getItem('refresh'))
              this.$store.dispatch(this.storePrefix + 'view/grid/refresh', refresh)
