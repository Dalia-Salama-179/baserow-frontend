  ...
  methods: {
      ...
      try {
        ...
        if (
          ((this.table.name == 'organizations' ||
            this.table.name == 'Organization_POC' ||
            this.table.name == 'Organization_R2') &&
    ...
    async pasteData(data, rowIndex, fieldIndex) {
      ...
      if (
        this.endSelect &&
        this.startSelect &&
        this.endSelect.type == this.startSelect.type &&
        this.startSelect.type == 'link_row'
      ) {
        const rows =
          this.$store.getters[this.storePrefix + 'view/grid/getAllRows']
        ...
        try {
          ...
          for (const item in dataNew) {
            ...
            // console.log('rowsrowsrows', typeof rows[rowIndex])
            if (typeof rows[rowIndex] === 'undefined') {
              // console.log('undefined == undefined == undefined == undefined == undefined');
              const createNew = await this.$store.dispatch(
                this.storePrefix + 'view/grid/createNewRow',
                {
                  view: this.view,
                  table: this.table,
            ...
            let searchValue
            if (this.isJson(dataNew[item][0])) {
              searchValue = JSON.parse(dataNew[item][0]).value[0].value
            } else {
              searchValue = dataNew[item][0]
            }
            ...
            if (data.results.length) {
              ...
              const newArray = [...data.results]
              const bigCities = newArray.filter(function (e) {
                return e[`${Object.keys(data.results[0])[2]}`] === searchValue
              })
              // console.log(bigCities);
              if (bigCities.length) {
                // console.log(newRow);
                const obj = [...newRow[`field_${this.startSelect.id}`]]
                // console.log('obj>>>', obj);
                obj.unshift({
                  id: bigCities[bigCities.length - 1].id,
                  value:
