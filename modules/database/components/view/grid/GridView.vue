...
<template>
<div v-scroll="scroll" class="grid-view" :class="{ 'grid-view--disable-selection': isMultiSelectActive }">
    <Scrollbars ref="scrollbars" horizontal="getHorizontalScrollbarElement" vertical="getVerticalScrollbarElement" :style="{ left: leftWidth + 'px' }" @vertical="verticalScroll" @horizontal="horizontalScroll"></Scrollbars>
    <GridViewSection ref="left" class="grid-view__left" :fields="leftFields" :decorations-by-place="decorationsByPlace" :table="table" :view="view" :include-field-width-handles="false" :include-row-details="true" :include-grid-view-identifier-dropdown="true" :read-only="readOnly" :store-prefix="storePrefix" :style="{ width: leftWidth + 'px' }" @refresh="$emit('refresh', $event)" @field-created="fieldCreated" @row-hover="setRowHover($event.row, $event.value)" @row-context="showRowContext($event.event, $event.row)" @row-dragging="rowDragStart" @cell-mousedown-left="multiSelectStart" @cell-mouseover="multiSelectHold" @cell-mouseup-left="multiSelectStop" @add-row="addRow()" @update="updateValue" @paste="multiplePasteFromCell" @edit="editValue" @selected="selectedCell" @unselected="unselectedCell" @select-next="selectNextCell" @edit-modal="$refs.rowEditModal.show($event.id)" @scroll="scroll($event.pixelY, 0)">
        <template #foot>
    ...
    <div ref="divider" class="grid-view__divider" :style="{ left: leftWidth + 'px' }"></div>
    <GridViewFieldWidthHandle class="grid-view__divider-width" :style="{ left: leftWidth + 'px' }" :grid="view" :field="primary" :width="leftFieldsWidth" :read-only="readOnly" :store-prefix="storePrefix"></GridViewFieldWidthHandle>
    <GridViewSection ref="right" class="grid-view__right" :fields="visibleFields" :decorations-by-place="decorationsByPlace" :table="table" :view="view" :include-add-field="true" :can-order-fields="true" :read-only="readOnly" :store-prefix="storePrefix" :style="{ left: leftWidth + 'px' }" @refresh="$emit('refresh', $event)" @field-created="fieldCreated" @row-hover="setRowHover($event.row, $event.value)" @row-context="showRowContext($event.event, $event.row)" @add-row="addRow()" @update="updateValue" @paste="multiplePasteFromCell" @edit="editValue" @cell-mousedown-left="multiSelectStart" @cell-mouseover="multiSelectHold" @cell-mouseup-left="multiSelectStop" @selected="selectedCell" @unselected="unselectedCell" @select-next="selectNextCell" @edit-modal="$refs.rowEditModal.show($event.id)" @scroll="scroll($event.pixelY, $event.pixelX)">
        <template #foot>
            <div v-if="publicGrid" class="grid-view__foot-logo">
                <a href="https://baserow.io" target="_blank" title="Baserow - open source no-code database tool and Airtable alternative">
                    <img height="14" src="@baserow/modules/core/static/img/logo.svg" alt="Baserow - open source no-code database tool and Airtable alternative" />
                </a>
            </div>
        </template>
    </GridViewSection>
    <GridViewRowDragging ref="rowDragging" :table="table" :view="view" :primary="primary" :fields="visibleFields" :store-prefix="storePrefix" vertical="getVerticalScrollbarElement" @scroll="scroll($event.pixelY, $event.pixelX)"></GridViewRowDragging>
    <Context ref="rowContext">
        <ul v-show="isMultiSelectActive" class="context__menu">
            ...
            <li>
                <a :class="{ 'context__menu-item--loading': deletingRow }" @click.stop="deleteRowsFromMultipleCellSelection()">
    ...
    <RowEditModal ref="rowEditModal" :table="table" :primary="primary" :visible-fields="[primary].concat(visibleFields)" :hidden-fields="hiddenFields" :rows="allRows" :read-only="readOnly" :show-hidden-fields="showHiddenFieldsInRowModal" @toggle-hidden-fields-visibility="
        showHiddenFieldsInRowModal = !showHiddenFieldsInRowModal
      " @update="updateValue" @toggle-field-visibility="toggleFieldVisibility" @order-fields="orderFields" @hidden="rowEditModalHidden" @field-updated="$emit('refresh', $event)" @field-deleted="$emit('refresh')" @field-created="fieldCreated"></RowEditModal>
</div>
</template>
...
import {
    notifyIf
} from '@baserow/modules/core/utils/error'
    ...
    beforeCreate() {
        this.$options.computed = {
            ...
            ...mapGetters({
                ...
                isMultiSelectActive: this.$options.propsData.storePrefix + 'view/grid/isMultiSelectActive',
            }),
        }
    },
    created() {
    ...
    methods: {
        ...
        async exportMultiSelect(event) {
            ...
            try {
                ...
                if (output !== undefined) {
                    const tsv = this.$papa.unparse(output, {
                        delimiter: '\t'
                    })
            ...
            if (this.isField && this.isField.type == "link_row" && this.isRow && this.stopRow && this.isRow.id == this.stopRow.id) {
                try {
                    ...
                    if (data.results.length) {
                        ...
                        const {
                            data
                        } = await FieldService(this.$client).fetchAll(
            ...
            if (this.endSelect && this.startSelect && this.endSelect.type == this.startSelect.type && this.startSelect.type == 'link_row') {
                ...
                try {
                    ...
                    for (let item in dataNew) {
                        ...
                        if (data.results.length) {
                            // console.log('newRow', Object.keys(data.results[0])[2]);
                            let newArray = [...data.results]
                            let bigCities = newArray.filter(function (e) {
                            ...
                            if (bigCities.length) {
                                // console.log(newRow);
                                let obj = [...newRow[`field_${this.startSelect.id}`]]
                                // console.log('obj>>>', obj);
                                obj.unshift({
                                    id: bigCities[bigCities.length - 1].id,
                                    value: bigCities[bigCities.length - 1][Object.keys(bigCities[bigCities.length - 1])[2]]
                            ...
                            // console.log("result Create NEW ",resultCreate);
                            let obj = [...newRow[`field_${this.startSelect.id}`]]
                            obj.unshift({
                                id: resultCreate.id,
                                value: resultCreate[Object.keys(resultCreate)[2]]
                            ...
                            // console.log('resultCreate ====3',resultCreate);
                            let obj = [...newRow[`field_${this.startSelect.id}`]]
                            obj.unshift({
                                id: resultCreate.id,
                                value: resultCreate[Object.keys(resultCreate)[2]]
